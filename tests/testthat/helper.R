...
numerical_gradient = function(func, x, dx = .Machine$double.eps^(1/3)){

    n_param = length(x)
    numerical_grad = rep(0, n_param)

    for (i in 1:n_param) {
        x_delta = rep(0, n_param)
        x_delta[i] = dx
        numerical_grad[i] = (func(x + x_delta) - func(x - x_delta)) / (2 * dx)
    }
